...
def test_create(api_factory, db_factory, env_config):
    ...
    # 调用创建订单接口，返回响应
    resp = order_step.create_order(create_body)
...
def test_distribute(api_factory, db_factory, env_config):
    ...
    # 调用创建订单接口，返回响应
    resp = order_step.distribute_order(distribute_body)
...
def test_stash(api_factory, db_factory, env_config):
    ...
    # 调用暂存订单接口，返回响应
    resp = order_step.stash_order(stash_boody)
    logging.info("暂存成功")
...
def test_submit(api_factory, db_factory, env_config):
    ...
    # 调用提交订单接口，返回响应
    resp = order_step.submit_order(submit_body)
...
def test_generate_sub_order(api_factory, db_factory, env_config):
    ...
    resp = order_step.generate_sub_order(generate_sub_order_body)
    # 查询数据库，校验是否落库
    record = order_db.query_by_bl_no(bl_no)
...
def test_fee_add(api_factory, db_factory, env_config):
    ...
    # 六、录入订单费用
    data_path = get_project_root() / f"data/{env_name}/order/fee.yaml"
    data_template = read_yaml(data_path)

    # 深拷贝请求模板，防止原始yaml被篡改
    fee_add_body = copy.deepcopy(data_template)
    # 使用新建产生的order_id分发
    fee_add_body["order_id"] = order_id
    # 处理对客对商费用，生成unique_id，进行费用关联
    ...
    for idx, cust_row in enumerate(customer_list):
        ...
        # 只有索引还在供应商列表范围内，才同步赋值
        if idx < len(supplier_list):
            supplier_list[idx]["unique_id"] = uid

    # 调用订单接口，返回响应
    resp = order_step.fee_add(fee_add_body)

    # 查询数据库，校验是否穿行
    rows = order_db.query_fee_by_order_id(order_id)
    count = len(rows)
    # 业务断言
    equal(count, 16, "订单穿行异常")
